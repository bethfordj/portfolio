...
function isAdmitted(gpa = 1.0, satScore = 0, recommendation = false) {
  if(gpa > 4.0) {
    return true;
  }
...
function useParameterToFilterArray (filterFunction) {
    let number = [];
    number = unfilteredArray.filter(filterFunction);
...
function makeHappy(array) {
  return array.map(string => {return 'Happy ' + string});
}
...
function getFullAddressesOfProperties(arrayOfObjects) {
  return arrayOfObjects.map(object => {return object.streetNumber + ' ' + object.streetName + ' ' + object.streetType + ' ' + object.city + ' ' + object.state + ' ' + object.zip});
}
