...
-- ORDERING RESULTS
-- **********************
SELECT * FROM country ORDER BY continent ASC, region DESC, population ASC ;
...
-- ROUNDING
-- **********************
select round(gnp/population, 5) FROM country WHERE gnp > 0;
...
select language, COUNT(*) AS number_of_countries from countrylanguage
GROUP BY language
...
SELECT district, SUM(population), AVG(population), MIN(population), MAX(population), COUNT(population) FROM city
WHERE countrycode = 'USA'
GROUP BY district
ORDER BY district;
...
SELECT district, SUM(population) AS total_city_population, COUNT(population) AS number_of_cities, AVG(population) AS average_population
FROM city
GROUP BY district
